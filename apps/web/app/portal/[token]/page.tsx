...
const INVOICE_BADGE_CLASSES: Record<string, string> = {
...
export default async function CustomerPortalPage({ params }: PageProps) {
  ...
  const [
    ...
    admin.from('customer_equipment').select('*').eq('customer_id', customer_id),
    admin
      .from('jobs')
      .select('*, users!jobs_technician_id_fkey(full_name)')
      .eq('customer_id', customer_id)
    ...
    admin.from('organizations').select('name, phone, email').eq('id', organization_id).single(),
  ])
                ...
                <li key={inv.id} className="px-5 py-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <span
                        className={`text-xs font-medium px-2.5 py-1 rounded-full ${INVOICE_BADGE_CLASSES[inv.status] ?? 'bg-gray-100 text-gray-700'}`}
                      >
